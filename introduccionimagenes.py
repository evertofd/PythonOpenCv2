...
_,countours,_=cv2.findContours(mascara1, cv2.RETR_TREE,cv2.CHAIN_APPROX_SIMPLE)
...
#Obtener las dimensiones de la mascara
m,n=mascara1.shape
ar=np.zeros((m,n))
...
_,countours,_=cv2.findContours(mascaraConvex, cv2.RETR_TREE,cv2.CHAIN_APPROX_SIMPLE)

cv2.drawContours(img, countours, -1,(0,0,255),1)

_,countours,_=cv2.findContours(mascaraRect, cv2.RETR_TREE,cv2.CHAIN_APPROX_SIMPLE)
...
#plt.hist(verde, bins=1000, histtype="stepfilled", color="verde")
#plt.hist(azul, bins=1000, histtype="stepfilled", color="azul")
